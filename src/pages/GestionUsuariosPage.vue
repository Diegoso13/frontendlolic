  ...
  <q-page class="ofi-page">
    ...
    <div class="ofi-header">
      <div>
        ...
        <h1 class="ofi-header__title">Gestion de Usuarios</h1>
        <p class="ofi-header__sub">{{ total }} Usuarios</p>
      </div>
    </div>
...
const { usuarios,total, pagination, loading,filtros, listar, eliminar, buscarConDebounce } =  useUsuarios()
...
function irEditar() {
  ...
  router.push({
    ...
    query: {
      modo: 'editar',
      username: selected.value[0].username
    }
  })
}
...
function confirmarEliminar() {
  ...
  const usuario = selected.value[0]

  $q.dialog({
    title: 'Confirmar eliminación',
    message: `¿Desea eliminar el usuario ${usuario.username}?`,
    cancel: true,
    persistent: true
  }).onOk(async () => {
